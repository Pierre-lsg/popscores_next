<script lang="ts">
  ...
  $: ranking = [...players].sort((a, b) => 
    getRelativeScore(a.scores, $holesStore) - getRelativeScore(b.scores, $holesStore)
  );
</script>
...
<style>
  ...
  .rank-1 { height: 80px; border: 2px solid #ffd700; order: 2; }
  .rank-2 { height: 60px; border: 2px solid silver; order: 1; }
  .rank-3 { height: 40px; border: 2px solid #cd7f32; order: 3; }
</style>