<script lang="ts">
  ...
  function nextCard(nextStep: 'session' | 'players' | 'holes' | 'scoring' | 'ranking') {
    currentStep = nextStep;
     if (nextStep === 'session') { step = 1; gameStatus.set('setup'); }
     else if (nextStep === 'players') { step = 2; gameStatus.set('setup'); }
     else if (nextStep === 'holes') { step = 3; gameStatus.set('setup'); }
     else if (nextStep === 'scoring') { step = 4; gameStatus.set('in_progress'); }
     else if (nextStep === 'ranking') { step = 5; gameStatus.set('finished'); }
  }
...
<div class="mobile-wizard">
  ...
  {:else if currentStep === 'holes'}
    <div class="step-content" in:slide>
      ...
      <table>
          ...
          {#each $holesStore as hole, i}
            <tr class="scroll-area">
              ...
              <td>  
                <button class="btn-delete" on:click={() => confirmDeleteHole(i)}>
                    &times;
                </button>
              </td>
            </tr>
      ...
      <div class="actions">
        <button on:click={() => nextCard('players')} class="btn btn-prev">Retour : Les joueurs</button>
        <button on:click={() => nextCard('scoring')} class="btn btn-next">Commencer la partie</button>
      </div>
    </div>
      ...
      <div class="scores-grid">
        {#each $playersStore as player}
          <div class="score-row">
            ...
            <Stepper 
              bind:value={player.scores[activeHoleIndex]} 
              min={1} 
            />
          </div>
        {/each}
  ...
  {:else if currentStep === 'ranking'}
    <div class="step-content" in:slide>
       ...
       <button class="btn btn-primary" on:click={() => saveGameToHistory()}>Sauvegarder la partie</button>
    </div>
...
<style>
  .mobile-wizard { padding: 1rem; display: flex; flex-direction: column; gap: 1rem; }
  ...
  .par-badge { background: var(--primary-light); color: var(--primary); padding: 2px 8px; border-radius: 4px; font-weight: bold; }

  .score-row { 
    display: flex; justify-content: space-between; align-items: center;
    background: var(--bg-card); padding: 0.8rem; border-radius: 8px; margin-bottom: 0.5rem;
  ...
  .btn-prev { background-color: gray; } 
